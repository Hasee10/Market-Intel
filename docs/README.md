...
| [`MOBILE_API_CHANGELOG.md`](MOBILE_API_CHANGELOG.md) | Dated record of every change to the `/api/mobile/*` contract, newest first, each entry saying whether the client has to change anything | Send to the mobile developer whenever a mobile route or its response changes. Read before assuming an old integration still matches |
