...
// Zero-magic-bytes checks - just enough to prove both renderers produce a
// real OOXML/PDF container from a snapshot without throwing, across the
// floor case (almost no data) and a data-rich case (many sections,
// paginated table), i.e. the exact two ends of the "dynamic slide count"
// rule in docs/reports-v2-architecture.md §5.
...
describe('buildReportDeck (PPTX)', () => {
  it('renders a floor-case snapshot (almost no data) without throwing', async () => {
    const buffer = await buildReportDeck(baseSnapshot());
    expect(buffer.length).toBeGreaterThan(0);
    // .pptx is a zip container - starts with the PK magic bytes
    expect(buffer[0]).toBe(0x50);
    expect(buffer[1]).toBe(0x4b);
  });

  it('renders a data-rich snapshot with a paginated competitor table without throwing', async () => {
...
describe('buildReportPdf', () => {
  it('renders a floor-case snapshot without throwing', async () => {
    const buffer = await buildReportPdf(baseSnapshot());
    expect(buffer.length).toBeGreaterThan(0);
    // PDF files start with "%PDF"
    expect(buffer.subarray(0, 4).toString('ascii')).toBe('%PDF');
  });

  it('renders a data-rich snapshot without throwing', async () => {
